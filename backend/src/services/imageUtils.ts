...
function getBase64Data(imageBase64: string): { buffer: Buffer; hadDataUrl: boolean } {
 ...
 */
export async function resizeAndCompressImage(imageBase64: string): Promise<string> {
  ...
  if (buffer.length === 0) return imageBase64;
